// Cloudflare Workers API - 第三部分：收藏管理和搜索历史

// 获取用户收藏
async function handleGetFavorites(request, db, userId) {
  try {
...
// 添加收藏
async function handleAddFavorite(request, db, userId) {
  try {
...
// 移除收藏
async function handleRemoveFavorite(request, db, userId, trackId) {
  try {
    const result = await db.prepare(
...
// 检查是否已收藏
async function handleCheckFavorite(request, db, userId) {
  try {
    const body = await request.json();
    const { track_id } = body;
...
// 获取搜索历史
async function handleGetSearchHistory(request, db, userId) {
  try {
    const url = new URL(request.url);
...
// 添加搜索历史
async function handleAddSearchHistory(request, db, userId) {
  try {
    const body = await request.json();
...
// 清空搜索历史
async function handleClearSearchHistory(request, db, userId) {
  try {
...
// 获取热门搜索
async function handleGetHotSearches(request, db) {
  try {
    const { results } = await db.prepare(`
