// Cloudflare Workers API - 第四部分：同步平台和音乐搜索

// 测试音乐源
async function handleTestMusicSources(request) {
  try {
...
// 音乐搜索代理
async function handleMusicSearch(request) {
  try {
...
// 获取同步平台列表
async function handleGetSyncPlatforms(request, db, userId) {
  try {
...
// 添加同步平台
async function handleAddSyncPlatform(request, db, userId) {
  try {
    const body = await request.json();
    const {
      platform,
...
// 移除同步平台
async function handleRemoveSyncPlatform(request, db, userId, platformId) {
  try {
    const result = await db.prepare(
...
// 同步平台数据
async function handleSyncPlatformData(request, db, userId, platformId) {
  try {
    // 获取平台信息
